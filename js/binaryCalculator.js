...
function circularShift(direction) {
  ...
  const shiftValue = parseInt(
    prompt(
      `Enter the number of positions to ${
...
function circularShiftRight(input, shift) {
  ...
  const shifted =
    input.substr(length - (shift % length)) +
    input.substr(0, length - (shift % length));
  return shifted;
}
...
function circularShiftLeft(input, shift) {
  ...
  const length = input.length;
  const shifted =
    input.substr(shift % length) + input.substr(0, shift % length);
  return shifted;
}
...
function binaryToFloat() {
  ...
  let mantissa =
    ...
    binary
      .substr(9)
      .split("")
      .reduce((acc, bit, i) => acc + bit * Math.pow(2, -(i + 1)), 0);
